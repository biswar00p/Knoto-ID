...
OUTPUT_FILE="planar_arrow_knotoid_polynomial_outputs.txt"
...
for file in "$INPUT_DIR"/*.xyz; do
    echo "Processing $file with --closure-method=$CLOSURE_METHOD"
    filename=$(basename "$file")
    echo "===== $filename =====" >> "$OUTPUT_FILE"
    $EXECUTABLE --planar --closure-method="$CLOSURE_METHOD" --arrow-polynomial "$file" >> "$OUTPUT_FILE"
    echo -e "\n" >> "$OUTPUT_FILE"
done
